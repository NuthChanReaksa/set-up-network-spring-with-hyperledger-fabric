...
public class FabricCAService {
    ...
    public void createAdminUserOrg1() throws Exception{
        ...
        if (tlsEnabled){
            // we will configure tls properties for the HFCA client
            // 2.1 configure tls
            File pemFile = new File(org1CertificatePath);
            if (!pemFile.exists()){
                throw new Exception("Certificate org1 CA file does not exits");
            }
            // Verify type of the certificate && validity of the certificate file
            props.setProperty("pemFile", org1CertificatePath);
            props.setProperty("allowAllHostNames", "true"); // to allow all host name
